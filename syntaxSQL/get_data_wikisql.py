    ...
    while i < len(lines):
        ...
        if line.startswith("P:"):
            ...
            if line != '' and len(line) != 0:
                questions.append(line.lstrip().rstrip())
            has_prefix = True
        if (line.startswith("select") or line.startswith("SELECT") or line.startswith("Select") or line.startswith("with") or line.startswith("With") or line.startswith("WITH")) and has_prefix:
            sql = [line]
            i += 1
            while i < len(lines):
                line = lines[i]
                ...
                line = line.replace("\r","")
                line = line.replace("\n","")
                if len(line) == 0 or len(line.strip()) == 0 or ord('0') <= ord(line[0]) <= ord('9') or not (line[0].isalpha() or line[0] in ['(',')','=','<','>', '+', '-','!','\'','\"','%']):
                    break
                sql.append(line)
                i += 1
            sql = " ".join(sql)
